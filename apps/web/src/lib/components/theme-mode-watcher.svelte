<script lang="ts">
  import { ModeWatcher, mode, setTheme, theme } from 'mode-watcher'
  import type { ComponentProps } from 'svelte'

  type ModeWatcherProps = ComponentProps<ModeWatcher>
  ...
  let { defaultTheme, ...modeWatcherProps } = props
  ...
  const setInitialThemeArgs = JSON.stringify(props)

  /**
   * @example "(function setInitialTheme(args) { ... }) (args)"
   */
  const setInitialThemeInvocation = `(${setInitialThemeDeclaration})(${setInitialThemeArgs})`
