...
```svelte notwoslash title="Responsive" showLineNumbers /-ml-2/ /pl-2/ /md:-ml-4/ /md:pl-4/
<Carousel.Root>
...
```svelte notwoslash showLineNumbers /vertical | horizontal/
...
```svelte notwoslash showLineNumbers {2-5}
<Carousel.Root
  opts={{
    align: "start",
    loop: true,
...
```svelte notwoslash showLineNumbers {2,5,19}
<script lang="ts">
...
```svelte notwoslash showLineNumbers {2,5,7-13,16}
<script lang="ts">
  import { type CarouselAPI } from "$lib/components/ui/carousel/context.js";
  import * as Carousel from "$lib/components/ui/carousel/index.js";
...
```svelte notwoslash showLineNumbers {2,7-11}
<script lang="ts">
